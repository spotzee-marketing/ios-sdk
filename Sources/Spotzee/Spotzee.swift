...
public class Spotzee {
    ...
    public static let shared = Spotzee()
    ...
    private var store = UserDefaults(suiteName: "Spotzee")
    ...
    public static func initialize(
        ...
        return Self.shared.initialize(apiKey: apiKey, inAppDelegate: inAppDelegate, launchOptions: launchOptions)
    }
    ...
    ///
    /// - Parameters:
    ///     - identity: An object representing a Spotzee user identity
    ///
    ...
    @MainActor
    public func show(notification: SpotzeeNotification) async {
    ...
    public func consume(notification: SpotzeeNotification) async {
        do {
    ...
    public func dismiss(notification: SpotzeeNotification) async {
        await MainActor.run {
    ...
    public func handle(universalLink: URL) -> Bool {
        guard isSpotzeeDeepLink(url: universalLink.absoluteString),
    ...
    private func checkInit() {
        assert(self.config != nil, "You must initialize the Spotzee library before calling any methods")
    }
...
extension Spotzee: InAppModelViewControllerDelegate {
